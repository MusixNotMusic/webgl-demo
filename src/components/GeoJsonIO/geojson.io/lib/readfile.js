...
export function readDrop(event, callback) {
  return function () {
    const results = [];
    const errors = [];
    if (
      ...
      let remaining = event.dataTransfer.files.length;
      [].forEach.call(event.dataTransfer.files, (f) => {
...
export function readAsText(f, callback) {
  try {
...
export function readAsArrayBuffer(f, callback) {
  try {
    const reader = new FileReader();
...
export function readFile(f, text, callback) {
  ...
  if (!fileType) {
    ...
    const features = [];
    readAsArrayBuffer(f, (err, arraybuffer) => {
