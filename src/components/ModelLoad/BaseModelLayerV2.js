...
export default class BaseModelLayer extends BaseModelModel{
  ...
  _calcModelBounds (lngLatAlt, size) {
    const { lon, lat, alt } = lngLatAlt;

    const _alt = alt || 0;

    const dx = size.x / 111000;
    const dy = size.y / 111000;
    const dz = size.z;
   ...
   * @param {*} lngLatAltArr 
   */
  setCoordination(object, lngLatAltArr, scale = 1) {
    const size = this.getObjectSize(object);
    const bounds = this._calcModelBounds(lngLatAltArr, size);
