...
class TransformControls extends BaseModelLayer {

	constructor(map, camera, domElement ) {

		super('transform-controls', map);
		...
		this.modelSize = { x: 10000, y: 10000, z: 20000};
		this.position = { lon: 104, lat: 30};

		this.bounds = this._calcModelBounds(this.position);
	...
	initHelper () {
		...
		this._gizmo = _gizmo;
		this.setMeshBounds(this._gizmo, this.bounds);
		this.scene.add( _gizmo );

		const _plane = new TransformControlsPlane();
		this._plane = _plane;
		// this.setMeshBounds(this._plane, this.bounds);
		this.scene.add( _plane );

		const boxGeo = new BoxGeometry(1, 1, 1);
        const material = new MeshBasicMaterial({ color: 'orange', wireframe: false });
        const boxMesh = new Mesh(boxGeo, material);
        // this._box = boxMesh;
		this.setMeshBounds(boxMesh, this.bounds);

        this.scene.add(boxMesh);
		// this.setMeshBounds(this.scene, this.bounds);
	...
	addEventListener () {
		...
		this.map.on( 'movemove', this._onPointerHover );
		this.map.on( 'moveup', this._onPointerUp );
	}

	removeEventListener () {
		this.map.off( 'mousedown', this._onPointerDown );
		this.map.off( 'movemove', this._onPointerHover );
		this.map.off( 'moveup', this._onPointerUp );
	}
	...
	pointerMove( pointer ) {
		...
		console.log('planeIntersect.point ==>', planeIntersect.point)
...
function getPointer( event ) {
	...
	if ( event.originalEvent.target.pointerLockElement ) {
		...
		return {
			x: ( event.point.x / this.map.transform.width ) * 2 - 1,
    		y: 1 - ( event.point.y / this.map.transform.height ) * 2,
		};
...
function onPointerDown( event ) {
	...
	// 	event.originalEvent.target.setPointerCapture( event.pointerId );
