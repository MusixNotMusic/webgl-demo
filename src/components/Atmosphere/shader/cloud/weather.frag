...
uniform float       iTime;
uniform bool        status[NUM_OF_STATUS]; // cloudy 0x1 rain 0x02 snow 0x04 blot 0x08 
...
float snowSDF(vec3 p) {
    ...
    float snowSpeed = 0.5 + 0.05 * hash11(yRandom);//random rainfall speed

    //separating droplets in x,y,z dimension
    vec3 randomOffset = vec3(x*snapSize*.25, -iTime*(snowSpeed) + yRandom, z * snapSize * .25);

    //ELONGATED SPHERE WATER DROPLET
    p = repeat(p - randomOffset, vec3(snapSize));
    p.y *= 0.8; //elongated sphere effect

    float r = sdSphere( p, snowSize * .01);
...
vec4 rainRender(vec3 ro, vec3 rd, bool isRain)
{
    vec2 bounds = hitBox( ro, rd );
    if ( bounds.x > bounds.y ) discard;
...
float renderBolt(vec3 ro, vec3 rd, float start, float end, out vec4 glow) {
    vec2 bounds = hitBox( ro, rd );
    if ( bounds.x > bounds.y ) discard;
    ...
    float d = near;
    float dD = (far - near) / float(MAX_STEPS);
...
void main () {
    ...
    if (status[1]) {
        vec4 rain = rainRender(vOrigin, normalize(vDirection), true);
        color += rain;
    }

    if (status[2] ) {
        vec4 snow = rainRender(vOrigin, normalize(vDirection), false);
        color += snow;
    }

    if (status[3]) {
        vec4 glow = vec4(0);
        float dist = renderBolt(vOrigin, normalize(vDirection), -0.5, MAX_DIST, glow);
        color += glow;
    }
